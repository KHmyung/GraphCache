...
public:
	...
	off_t get_pg_offset() const { return (off_t) offset; }
	...
	// KH: APR page operations
	bool is_evicted() const {
		return evicted;
	}
	bool evicted_by_lifo() const {
		return is_evicted() && policy;
	}
	bool evicted_by_clock() const {
		return is_evicted() && !policy;
	}
	void set_evicted(bool pol) {
		evicted = true;
		this->policy = pol;
	}
	void clear_evicted() {
	...
	unsigned long get_stime() {
		return stime;
	}
	void set_stime(unsigned long time) {
		this->stime = time;
	}
