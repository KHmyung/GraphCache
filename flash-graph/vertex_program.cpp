...
void vertex_program::multicast_msg(vertex_id_t ids[], int num,
		vertex_message &msg)
{
	//std::cout << "vertex_program::multicast_msg1" << std::endl;
	assert(!msg.is_flush());
...
{
	//std::cout << "vertex_program::activate_vertices" << std::endl;
...
{
	//std::cout << "vertex_program::activate" << std::endl;
	worker_thread *curr = (worker_thread *) thread::get_curr_thread();
	assert(curr == t);
...
vertex_id_t vertex_program::get_vertex_id(const compute_vertex &v) const
{
	...
	//std::cout << "get_vertex_id:" << id << std::endl;
	return id;
}
